...
export default function Cart({ count, items, onhandelEvent }) {
  const [showModal, setShowModal] = useState(false);
  const [orderModal, setOrderModal] = useState(false);
  const handelModal = () => {
    setShowModal((previousState) => !previousState);
  };
  const handelSuccessModal = () => {
    setShowModal(false);
  ...
  return (
    <Fragment>
              ...
              {count > 0 ? (
                items.map((item) => {
                  return (
                    <CartItem
                      data={item}
                      onEmitDecreaseItem={(id) => onhandelEvent(id, -1)}
                      onEmitIncreaseItem={(id) => onhandelEvent(id, 1)}
                      key={item.id}
                    />
                  );
                })
            ...
            <div className="checkout-modal_footer">
              {count > 0 && (
                <>
                  ...
                  <button onClick={handelSuccessModal}>Order Now</button>
                </>
              )}
            </div>
          </div>
      ...
      {orderModal && <OrderSuccess onClose={handelSuccessModal} />}
    </Fragment>
  );
}
